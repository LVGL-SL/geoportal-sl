	...
	for(var n in nodes){
		...
		code.push(
			...
			((n1||n0[1][3]) ? 
				...
				(n0[1][5] ? 
					' class="action" onClick="showMenu(\'' + 
					strData + '[' + n + ']\', this, event)"' : ''
				) + 
				' src="' + jst_image_folder + '/' + 
				(n1 ? 
					(n0[1][3]?
						n0[1][3]:"closed_folder.png"
					) : n0[1][3] ? 
						n0[1][3] : "node.png"
				) + '" alt="">':''
			)+
...
function showMenu(strData, img, event){
	...
	if($("#body_old").length == 0) {
		jst_cm.style.top = (offY + event.clientY -160) + "px";
		jst_cm.style.left = (offX + event.clientX - 40) + "px";	
	}
	else {
